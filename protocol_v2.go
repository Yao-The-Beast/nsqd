		...
		case msg := <-memoryMsgChan:
			...
			if client.Channel.name == "0#ephemeral" {
				...
				if messagesReceived < 1000 {
					...
					client.ctx.nsqd.logf("Yao: Message Pump Underlying Buffer has a size of: %d, Msg Id: %d, Msg size is: %d", client.Writer.Buffered() / len(msg.Body), len(msg.Body), messagesReceived)
				}
                messagesReceived++;
                //write to file
                if messagesReceived == 1000 {
